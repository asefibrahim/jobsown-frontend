        ...
        <div className="max-w-5xl mx-auto min-h-80">
            ...
            <div className="mt-5 mb-2 rounded-lg">
              <ul className="flex justify-between px-20 py-4 bg-[#F3F3F3] text-[#000099] font-semibold rounded-lg">
                {
                  tabsItems?.map(item => (
                    <label htmlFor={item}><li onClick={() => setCurrentTab(item)} className={`uppercase ${currentTab === item && 'border-b-2 border-[#000099]'}`}>{item}</li></label>
                  ))
                }
              </ul>
            </div>
            <div role="tablist" className="tabs tabs-bordered">
              <input type="radio" id="overview" name="my_tabs_1" role="tab" className="tab hidden" aria-label="OVERVIEW" defaultChecked />
              ...
              <input type="radio" id="education" name="my_tabs_1" role="tab" className="tab hidden" aria-label="EDUCATION" />
              <div role="tabpanel" className="tab-content">
                <EducationCard education={userDetails?.education} />
              </div>

              <input type="radio" id="skills" name="my_tabs_1" role="tab" className="tab hidden" aria-label="SKILL" />
              <div role="tabpanel" className="tab-content">
                <SkillsCard skills={userDetails?.skills} />
              </div>
              <input type="radio" id="experiences" name="my_tabs_1" role="tab" className="tab hidden" aria-label="EXPERIENCE" />
              <div role="tabpanel" className="tab-content">
                <WorkExperiences experiences={userDetails?.experience} />
              </div>
              <input type="radio" id="contact" name="my_tabs_1" role="tab" className="tab hidden" aria-label="CONTACT" />
              <div role="tabpanel" className="tab-content">
                <ContactCard
                phoneNumber={userDetails?.phone_number}
                email={userDetails?.email}
